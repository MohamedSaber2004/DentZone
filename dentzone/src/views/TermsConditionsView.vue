...
const fetchTerms = (lang: number): Promise<string> => policyRepository.getTerms(lang)
...
const load = async () => {
  ...
  try {
    try {
      html.value = await fetchTerms(locale.value === 'ar' ? API_LANG.ARABIC : API_LANG.ENGLISH)
    } catch {
      html.value = await fetchTerms(API_LANG.ARABIC)
    }
  } catch {
    failed.value = true
  } finally {
  ...
  <div class="page">
    <div class="container page__inner">
      ...
      <div v-else-if="failed" class="page__state">
        ...
        <h2 class="page__state-title">{{ t('terms.errorTitle') }}</h2>
        <p class="page__state-desc">{{ t('terms.errorDescription') }}</p>
