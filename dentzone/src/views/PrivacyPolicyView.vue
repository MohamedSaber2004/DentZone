...
const fetchPolicy = (lang: number): Promise<string> => policyRepository.getPolicy(lang)
...
const load = async () => {
  ...
  try {
    try {
      html.value = await fetchPolicy(locale.value === 'ar' ? API_LANG.ARABIC : API_LANG.ENGLISH)
    } catch {
      html.value = await fetchPolicy(API_LANG.ARABIC)
    }
  } catch {
    failed.value = true
  } finally {
...
<template>
  <div class="page">
    <div class="container page__inner">
      ...
      <h1 class="page__title">{{ t('policy.title') }}</h1>
      ...
      <div v-else-if="failed" class="page__state">
        ...
        <h2 class="page__state-title">{{ t('policy.errorTitle') }}</h2>
        <p class="page__state-desc">{{ t('policy.errorDescription') }}</p>
